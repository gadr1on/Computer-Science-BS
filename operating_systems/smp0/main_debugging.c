...
//WordCountEntry is a structure type , who holds 2 information.
// 1.*word a pointer to a character string and counter value.
...
//Here argc will contain the number of argument passed and argv will contain
// pointer to those argument.
//./main cat nap dog
...
int main(int argc, char **argv)
{
  ...
  argv++;
  //Loop through the argv, till it encounters null value, that menas the loop will start
  // from cat and will end after the word dog.
  while (*argv != NULL) {
    // Check for the -h option, it checks if argv starts from '-'
    if (**argv == '-') {
      // Below switch case the checks the character after '-'. It is checked only if '-'
      // option is used
      switch ((*argv)[1]) {
        case 'h':
          printHelp(prog_name);
      ...
      if (entryCount < LENGTH(entries)) {
        ...
        //entries[entryCount++].counter = 0; addressing to the question 2.
        // Plese check there for the explanation
